...
## External theme hook (optional)

`crystal-theme.widget` draws nothing. It polls an optional CSS file every
2 seconds and injects its contents into the shared Übersicht document as
CSS custom properties, which the other widgets reference as
`var(--crystal-static, <shipped colour>)` and the like. When the file is
absent every widget keeps drawing its own colours exactly as before, so
the widget is safe to leave installed — or delete it if you have no use
for it.
